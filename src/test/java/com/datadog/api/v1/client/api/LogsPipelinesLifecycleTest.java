...
public class LogsPipelinesLifecycleTest extends V1ApiTest {
    ...
    public void pipelineLifecycleTest() throws ApiException {
        ...
        // Create a pipeline
        LogsProcessor grokParser = new LogsProcessor(
            new LogsGrokParser()
                .source("src")
        ...
        LogsProcessor logDateRemapper = new LogsProcessor(new LogsDateRemapper().addSourcesItem("source").name("log date remapper"));
        LogsProcessor logStatusRemapper = new LogsProcessor(new LogsStatusRemapper().addSourcesItem("source").name("log status remapper"));
        LogsProcessor serviceRemapper = new LogsProcessor(new LogsServiceRemapper().addSourcesItem("source").name("service remapper"));
        LogsProcessor logMessageRemapper = new LogsProcessor(
            new LogsMessageRemapper().addSourcesItem("source").name("log message remapper")
        );
        LogsProcessor remapperToAttribute = new LogsProcessor(
        ...
        LogsProcessor remapperToTag = new LogsProcessor(
            new LogsAttributeRemapper()
                .addSourcesItem("source")
                .sourceType("tag")
                .target("target")
        ...
        LogsProcessor urlParser = new LogsProcessor(new LogsURLParser().addSourcesItem("source").target("target").name("URL parser"));
        LogsProcessor userAgentParser = new LogsProcessor(
            new LogsUserAgentParser().addSourcesItem("source").target("target").isEncoded(true).name("user agent parser")
        );
        LogsProcessor categoryProcessor = new LogsProcessor(
            new LogsCategoryProcessor()
                .addCategoriesItem(new LogsCategoryProcessorCategory().name("category").filter(new LogsFilter().query("query")))
                .target("target")
                .name("category processor")
        );
        LogsProcessor arithmeticProcessor = new LogsProcessor(
            new LogsArithmeticProcessor().expression("foo + bar").target("target").isReplaceMissing(true).name("arithmetic processor")
        );
        LogsProcessor stringBuilderProcessor = new LogsProcessor(
            new LogsStringBuilderProcessor().template("template").target("target").isReplaceMissing(true).name("string builder processor")
        );
        LogsProcessor geoIPParser = new LogsProcessor(
            new LogsGeoIPParser().addSourcesItem("source").target("target").name("geo IP parser")
        );
        LogsProcessor lookupProcessor = new LogsProcessor(
            new LogsLookupProcessor()
                .source("source")
                .target("target")
        ...
        LogsProcessor traceRemapper = new LogsProcessor(new LogsTraceRemapper().addSourcesItem("source").name("trace remapper"));
        LogsProcessor pipelineProcessor = new LogsProcessor(
