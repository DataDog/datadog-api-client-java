...
public class MonitorsApiTest extends V1ApiTest {
    ...
    private SyntheticsAPITest apiTestConfig = new SyntheticsAPITest()
        .config(
    ...
    public void listMonitorsTest() throws ApiException {
        ...
        for (String name : names) {
            Monitor monitor = new Monitor().name(name).type(testingMonitorType).query(testingMonitorQuery);
    ...
    public void monitorGetSyntheticsTest() throws ApiException {
        ...
        try {
            syntheticsApi.deleteTests(new SyntheticsDeleteTestsPayload().publicIds(Arrays.asList(synt.getPublicId())));
        } catch (ApiException e) {
            if (e.getCode() == 404) {
                // doesn't exist => ok
            } else {
    ...
    public void monitorGetErrorsTest() throws ApiException, IOException {
        ...
        try {
            fakeAuthApi.getMonitor(Long.valueOf(1234), new MonitorsApi.GetMonitorOptionalParameters().groupStates("notagroupstate"));
            fail("Expected ApiException not thrown");
        } catch (ApiException e) {
            assertEquals(403, e.getCode());
            APIErrorResponse error = objectMapper.readValue(e.getResponseBody(), APIErrorResponse.class);
    ...
    public void monitorDelete400ErrorTest() throws IOException {
        ...
        stubFor(delete(urlPathEqualTo(apiUri + "/121")).willReturn(okJson(fixtureData).withStatus(400)));
    ...
    public void monitorDelete401ErrorTest() throws IOException {
        String fixtureData = TestUtils.getFixture(fixturePrefix + "/error_401.json");
        stubFor(delete(urlPathEqualTo(apiUri + "/121")).willReturn(okJson(fixtureData).withStatus(401)));
        // Cannot trigger 401 for client. Need underrestricted creds. Mock it.
        try {
            unitApi.deleteMonitor(Long.valueOf(121));
            fail("Expected ApiException not thrown");
    ...
    public void monitorCanDeleteErrorsTest() throws ApiException, IOException {
        ...
        Monitor composite = new Monitor().name(name + "-composite").type(MonitorType.COMPOSITE).query(Long.toString(monitorId));
    ...
    public void monitorValidateErrorsTest() throws IOException {
        ...
        try {
            fakeAuthApi.validateMonitor(new Monitor().type(testingMonitorType).query(testingMonitorQuery));
