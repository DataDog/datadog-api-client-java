...
public class LogsApiTest extends V2APITest {
    ...
    public void listLogsTest() throws Exception {
        ...
        TestUtils.retry(
            ...
            () -> {
                try {
                    LogsListResponse response = api.listLogs(new LogsApi.ListLogsOptionalParameters().body(bothMessagesRequest));
                    ...
                    responseAscending.set(
                        api.listLogs(
                            new LogsApi.ListLogsOptionalParameters()
                                .body(new LogsListRequest().filter(allLogsFilter).sort(LogsSort.TIMESTAMP_ASCENDING))
                        )
                    );
            ...
            () -> {
                try {
                    responseDescending.set(
                        api.listLogs(
                            new LogsApi.ListLogsOptionalParameters()
                                .body(new LogsListRequest().filter(allLogsFilter).sort(LogsSort.TIMESTAMP_DESCENDING))
                        )
                    );
        ...
        LogsListResponse pageOneResponse = api.listLogs(
            new LogsApi.ListLogsOptionalParameters()
                .body(new LogsListRequest().filter(allLogsFilter).page(new LogsListRequestPage().limit(1)))
        );
        ...
        LogsListResponse pageTwoResponse = api.listLogs(
            new LogsApi.ListLogsOptionalParameters()
                .body(new LogsListRequest().filter(allLogsFilter).page(new LogsListRequestPage().cursor(cursor).limit(1)))
        );
