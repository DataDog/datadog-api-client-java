...
public class RecorderSteps {
    ...
    public void setupClock(Scenario scenario) throws java.io.IOException {
        if (TestUtils.getRecordingMode().equals(RecordingMode.MODE_IGNORE) || scenario.getSourceTagNames().contains("@integration-only")) {
        ...
        if (!TestUtils.getRecordingMode().equals(RecordingMode.MODE_REPLAYING)) {
            ...
            try {
                ...
                throw new IOException(
                    "Time file '" + freezeFile + "' not found: create one setting `RECORD=true` or ignore it using `RECORD=none`"
                );
            }
            world.now = OffsetDateTime.ofInstant(Instant.now(world.clock), ZoneOffset.UTC);
        }
        ...
        if (TestUtils.getRecordingMode().equals(RecordingMode.MODE_REPLAYING)) {
            ...
            if (!cassette.exists()) {
                throw new IOException(
                    "Cassette '" + cassette.getPath() + "' not found: create one setting `RECORD=true` or ignore it using `RECORD=none`"
                );
            }
        }
    }
    ...
    public void cleanAndSendExpectations(Scenario scenario) throws IOException {
        ...
        if (!TestUtils.getRecordingMode().equals(RecordingMode.MODE_RECORDING) || scenario.getStatus() == Status.SKIPPED) {
        ...
        for (LogEventRequestAndResponse requestAndResponse : requestsAndResponses) {
            ...
            for (Header header : headers) {
                if (
                    !header.getName().equals("DD-API-KEY") &&
            ...
            for (Parameter param : params) {
                if (!param.getName().equals("api_key") && !param.getName().equals("application_key")) cleanParams.add(param);
            }
