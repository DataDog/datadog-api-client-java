    ...
    for (Map.Entry<String, Given.Parameter> entry : givenParams.entrySet()) {
      if (givenParametersClass != null) {
        try {
          ...
          f.set(parameters, entry.getValue().resolve(f.getType(), context, getObjectMapper(), getVersion()));
        } catch (NoSuchFieldException e) {
          Class<?>[] types = givenOperation.getParameterTypes();
          Class fieldType = types[givenParametersArray.size()];
          givenParametersArray.add(entry.getValue().resolve(fieldType, context, getObjectMapper(), getVersion()));
        }
      } else {
        Class<?>[] types = givenOperation.getParameterTypes();
        Class fieldType = types[givenParametersArray.size()];
