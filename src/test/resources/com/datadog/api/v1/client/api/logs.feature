...
Feature: Logs
  ...
  Scenario: Search logs returns "OK" response
    Given a valid "appKeyAuth" key in the system
    And new "ListLogs" request
    And body with value {"index":"main","limit":5,"query":"service:web* AND @http.status_code:[200 TO 299]","sort":"asc","time":{"from":"2020-02-02T02:02:02Z","timezone":"Europe/Paris","to":"2020-02-02T20:20:20Z"}}
    When the request is sent
