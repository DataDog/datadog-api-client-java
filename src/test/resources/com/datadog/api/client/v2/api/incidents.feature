...
Feature: Incidents
  ...
  Scenario: Import an incident returns "CREATED" response
    Given operation "ImportIncident" enabled
    And new "ImportIncident" request
    And body with value {"data": {"type": "incidents", "attributes": {"declared": "2025-01-01T00:00:00Z", "detected": "2025-01-01T00:00:00Z", "fields": {"severity": {"value": "SEV-5"}, "state": {"value": "active"}}, "title": "{{unique}}", "visibility": "organization"}}}
    When the request is sent
    Then the response status is 201 CREATED
