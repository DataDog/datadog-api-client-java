 ...
 * @param usageType Usage type to retrieve. Usage types are in the format <code>&lt;usage_type&gt;_usage</code>. Example: <code>infra_host_usage</code> To obtain the complete list of active usage types that can be used to replace <code>&lt;usage_type&gt;</code> in the field names, make a request to the <a href="https://docs.datadoghq.com/api/latest/usage-metering/#get-usage-attribution-types">Get usage attribution types API</a>. (required)
...
@Deprecated
  ...
  public static class GetMonthlyUsageAttributionOptionalParameters {
    ...
    /**
     * Set sortName.
     * @param sortName The field to sort by. Sort fields are in the format <code>&lt;usage_type&gt;_usage</code>. Example: <code>infra_host_usage</code> To obtain the complete list of usage attribution types that can be used to replace <code>&lt;usage_type&gt;</code> in the field names, make a request to the <a href="https://docs.datadoghq.com/api/latest/usage-metering/#get-usage-attribution-types">Get usage attribution types API</a>. (optional)
     * @return GetMonthlyUsageAttributionOptionalParameters
     */
 ...
 * @param fields Comma-separated list of usage types to return, or <code>*</code> for all usage types. Usage types are in the format <code>&lt;usage_type&gt;_usage</code> and <code>&lt;usage_type&gt;_percentage</code>. Example: <code>infra_host_usage,infra_host_percentage</code> To obtain the complete list of usage attribution types that can be used to replace <code>&lt;usage_type&gt;</code> in the field names, make a request to the <a href="https://docs.datadoghq.com/api/latest/usage-metering/#get-usage-attribution-types">Get usage attribution types API</a>. (required)
 ...
 */
   ...
   * <p>For SDK users only: all fields on <code>UsageSummaryResponse</code>, <code>UsageSummaryDate</code>, and
   * <code>UsageSummaryDateOrg</code> are accessible through each object's <code>additionalProperties</code> map.
   ...
   * to enumerate every available key at each response level.</p>
   * <p>This endpoint is only accessible for <a href="https://docs.datadoghq.com/account_management/multi_organization/">parent-level organizations</a>.</p>
   *
   * @param startMonth Datetime in ISO-8601 format, UTC, precise to month: <code>[YYYY-MM]</code> for usage beginning in this month. Maximum of 15 months ago. (required)
   * @param parameters Optional parameters for the request.
