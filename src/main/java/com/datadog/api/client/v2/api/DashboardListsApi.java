 ...
 */
  ...
  public CompletableFuture<ApiResponse<DashboardListItems>> getDashboardListItemsWithHttpInfoAsync(Long dashboardListId) {
    ...
    Invocation.Builder builder;
    try {
      builder = apiClient.createBuilder("v2.DashboardListsApi.getDashboardListItems", localVarPath, new ArrayList<Pair>(), localVarHeaderParams, new HashMap<String, String>(), new String[] {"application/json" }, new String[] { "AuthZ", "apiKeyAuth", "appKeyAuth" });
    } catch (ApiException ex) {
      CompletableFuture<ApiResponse<DashboardListItems>> result = new CompletableFuture<>();
      result.completeExceptionally(ex);
      return result;
