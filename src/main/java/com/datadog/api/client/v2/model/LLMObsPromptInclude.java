...
/**
 * Includes messages from a specific version of another prompt. Omitting <code>items</code> includes
 * every message from that version in its original order. When <code>items</code> is present,
 * messages are included in the specified order, including repetitions.
 */
@JsonPropertyOrder({
...
public class LLMObsPromptInclude {
  ...
  /**
   * Zero-based indexes of messages from the included prompt. For example, <code>[2, 0, 0]</code>
   * includes the third message, followed by the first message twice. Omit this field to include all
