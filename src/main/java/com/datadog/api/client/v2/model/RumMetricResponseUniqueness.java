...
public class RumMetricResponseUniqueness {
  ...
  private RumMetricResponseUniquenessWhen when;

  public RumMetricResponseUniqueness() {}

  @JsonCreator
  public RumMetricResponseUniqueness(
      @JsonProperty(required = true, value = JSON_PROPERTY_WHEN)
          RumMetricResponseUniquenessWhen when) {
    this.when = when;
    this.unparsed |= !when.isValid();
  }

  public RumMetricResponseUniqueness when(RumMetricResponseUniquenessWhen when) {
    this.when = when;
    this.unparsed |= !when.isValid();
    return this;
  }
  ...
  public RumMetricResponseUniquenessWhen getWhen() {
    return when;
  }

  public void setWhen(RumMetricResponseUniquenessWhen when) {
    if (!when.isValid()) {
      this.unparsed = true;
    }
    this.when = when;
