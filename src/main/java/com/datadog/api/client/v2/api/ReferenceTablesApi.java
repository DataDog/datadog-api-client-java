...
public class ReferenceTablesApi {
   ...
   * Provide access_details in file_metadata pointing to a CSV file in cloud storage (AWS S3, Azure
   * Blob, or GCP Cloud Storage).
   *
   * @param body (required)
   ...
   * @throws ApiException if fails to make API call
   */
  public TableRowResourceArray getRowsByID(List<String> rowId, String id) throws ApiException {
    return getRowsByIDWithHttpInfo(rowId, id).getData();
  }
   ...
   */
  public CompletableFuture<TableRowResourceArray> getRowsByIDAsync(List<String> rowId, String id) {
    return getRowsByIDWithHttpInfoAsync(rowId, id)
        .thenApply(
            response -> {
              return response.getData();
            });
   ...
   *     </table>
   */
  public ApiResponse<TableRowResourceArray> getRowsByIDWithHttpInfo(List<String> rowId, String id)
      throws ApiException {
    Object localVarPostBody = null;
   ...
   * endpoint with the upload_id in file_metadata. For existing tables of type <code>
   * source: S3, GCS, or AZURE</code>, provide updated access_details in file_metadata pointing to a
   * CSV file in the same type of cloud storage.
   *
   * @param id The ID of the reference table to update (required)
   * @param body (required)
   * @return ApiResponse&lt;Void&gt;
