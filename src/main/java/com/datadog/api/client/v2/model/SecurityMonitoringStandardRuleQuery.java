...
public class SecurityMonitoringStandardRuleQuery {
  ...
  /**
   * <strong>This field is currently unstable and might be removed in minor version
   * upgrade.</strong> The index to run the query on, if the <code>dataSource</code> is <code>logs
   * </code>. Only used for scheduled rules, i.e. when the <code>schedulingOptions</code> field is
