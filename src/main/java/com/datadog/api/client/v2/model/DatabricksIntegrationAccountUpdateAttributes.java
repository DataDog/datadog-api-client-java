...
public class DatabricksIntegrationAccountUpdateAttributes {
   ...
   * Choosing <code>private_action_runner</code> leaves the <code>databricks-model-serving-metrics
   * </code> dataflow unable to collect data. <code>bearer_token</code> is deprecated on Databricks:
   * it is accepted only on accounts that already use it and never on creation, so it cannot move an
   * account onto token authentication. Migrate those accounts to <code>databricks_oauth</code> or
   * <code>private_action_runner</code>.
   *
   ...
   * created; on update, omitted fields keep their current values. Some dataflows have
   * prerequisites, noted on each; unless one is documented as rejecting the request, it is not
   * verified, so a dataflow enabled without it is stored but collects no data.
   *
   * @return dataflows
   */
  @jakarta.annotation.Nullable
