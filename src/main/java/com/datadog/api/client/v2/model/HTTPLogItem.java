...
public class HTTPLogItem {
   ...
   * it is indexed for full text search. - Messages truncate to 25 KiB for non-message fields, and
   * 75 KiB for the message field. - Queries match against the truncated version. - The truncated
   * version displays only when doing a grouped query, for example, logs grouped by that truncated
   * field or similar operations that display that specific field. - Datadog stores the full text
   * for the fields, and you can see the full log in regular list queries in the Logs Explorer.
   *
   * @return message
   */
