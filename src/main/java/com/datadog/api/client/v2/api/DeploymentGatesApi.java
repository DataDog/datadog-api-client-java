 ...
 */
   ...
   * <p>When the <code>configuration</code> attribute is provided, rules are evaluated inline from that configuration
   * and no pre-configured gate is required. When <code>configuration</code> is omitted, rules are resolved from the
   * gate pre-configured for the given service and environment through the Datadog UI, API, or Terraform.</p>
   *
   * @param body  (required)
