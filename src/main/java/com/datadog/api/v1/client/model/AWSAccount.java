...
public class AWSAccount {
    ...
    @ApiModelProperty(
        ...
        value = "An object, (in the form `{\"namespace1\":true/false, \"namespace2\":true/false}`), that enables or disables metric collection for specific AWS namespaces for this AWS account only."
    )
    ...
    @ApiModelProperty(
        example = "true",
        value = "Whether Datadog collects cloud security posture management resources from your AWS account. This includes additional resources not covered under the general `resource_collection`."
    )
    ...
    @ApiModelProperty(
        example = "[\"<KEY>:<VALUE>\"]",
        value = "The array of EC2 tags (in the form `key:value`) defines a filter that Datadog uses when collecting metrics from EC2. Wildcards, such as `?` (for single characters) and `*` (for multiple characters) can also be used. Only hosts that match one of the defined tags will be imported into Datadog. The rest will be ignored. Host matching a given tag can also be excluded by adding `!` before the tag. For example, `env:production,instance-type:c1.*,!region:us-east-1`"
    )
    ...
    @ApiModelProperty(
        example = "[\"<KEY>:<VALUE>\"]",
        value = "Array of tags (in the form `key:value`) to add to all hosts and metrics reporting through this integration."
    )
    ...
    public boolean equals(Object o) {
        ...
        return (
            Objects.equals(this.accessKeyId, awSAccount.accessKeyId) &&
            ...
            Objects.equals(this.cspmResourceCollectionEnabled, awSAccount.cspmResourceCollectionEnabled) &&
