 ...
 */
@ApiModel(
    description = "The downtime object definition of the active child for the original parent recurring downtime. This field will only exist on recurring downtimes."
)
@JsonPropertyOrder(
    {
...
public class DowntimeChild {
    ...
    @ApiModelProperty(
        example = "2",
        value = "`0` for a downtime applied on `*` or all, `1` when the downtime is only scoped to hosts, or `2` when the downtime is scoped to anything but hosts."
    )
    ...
    @ApiModelProperty(
        example = "1412793983",
        value = "POSIX timestamp to end the downtime. If not provided, the downtime is in effect indefinitely until you cancel it."
    )
    @JsonIgnore
    public Long getEnd() {
    ...
    @ApiModelProperty(
        ...
        value = "A message to include with notifications for this downtime. Email notifications can be sent to specific users by using the same `@username` notation as events."
    )
    ...
    @ApiModelProperty(
        example = "123456",
        value = "A single monitor to which the downtime applies. If not provided, the downtime applies to all monitors."
    )
    @JsonIgnore
    ...
    @ApiModelProperty(
        example = "[\"*\"]",
        value = "A comma-separated list of monitor tags. For example, tags that are applied directly to monitors, not tags that are used in monitor queries (which are filtered by the scope parameter), to which the downtime applies. The resulting downtime applies to monitors that match ALL provided monitor tags. For example, `service:postgres` **AND** `team:frontend`."
    )
    ...
    @ApiModelProperty(
        example = "1412792983",
        value = "POSIX timestamp to start the downtime. If not provided, the downtime starts the moment it is created."
    )
    ...
    @ApiModelProperty(
        ...
        value = "The timezone in which to display the downtime's start and end times in Datadog applications."
    )
    ...
    public boolean equals(Object o) {
        ...
        return (
            Objects.equals(this.active, downtimeChild.active) &&
