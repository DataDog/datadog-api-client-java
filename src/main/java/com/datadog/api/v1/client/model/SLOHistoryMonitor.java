 ...
 */
@ApiModel(
    description = "An object that holds an SLI value and its associated data. It can represent an SLO's overall SLI value. This can also represent the SLI value for a specific monitor in multi-monitor SLOs, or a group in grouped SLOs."
)
@JsonPropertyOrder(
    {
...
public class SLOHistoryMonitor {
    ...
    @javax.annotation.Nullable
    @ApiModelProperty(
        value = "For `monitor` based SLOs, this includes the aggregated history as arrays that include time series and uptime data where `0=monitor` is in `OK` state and `1=monitor` is in `alert` state."
    )
    ...
    @ApiModelProperty(
        example = "1615867200",
        value = "For `monitor` based SLOs, this is the last modified timestamp in epoch seconds of the monitor."
    )
    ...
    @ApiModelProperty(
        example = "string",
        value = "For groups in a grouped SLO, this is the group name. For monitors in a multi-monitor SLO, this is the monitor name."
    )
    ...
    @ApiModelProperty(
        example = "2",
        value = "The amount of decimal places the SLI value is accurate to for the given from `&&` to timestamp. Use `span_precision` instead."
    )
    ...
    @ApiModelProperty(
        example = "true",
        value = "For `monitor` based SLOs, when `true` this indicates that a replay is in progress to give an accurate uptime calculation."
    )
    ...
    @ApiModelProperty(
        example = "2",
        value = "The amount of decimal places the SLI value is accurate to for the given from `&&` to timestamp."
    )
    ...
    public boolean equals(Object o) {
        ...
        return (
            Objects.equals(this.errorBudgetRemaining, slOHistoryMonitor.errorBudgetRemaining) &&
