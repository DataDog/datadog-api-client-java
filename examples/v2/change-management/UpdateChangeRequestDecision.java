...
public class Example {
  public static void main(String[] args) {
    ...
    ChangeRequestDecisionUpdateRequest body = new ChangeRequestDecisionUpdateRequest()
.data(new ChangeRequestDecisionUpdateData()
.attributes(new ChangeRequestDecisionUpdateDataAttributes()
.id("CHM-1234"))
.relationships(new ChangeRequestDecisionUpdateDataRelationships()
.changeRequestDecisions(new ChangeRequestDecisionsRelationship()
.data(Collections.singletonList(new ChangeRequestDecisionRelationshipData()
.id("decision-id-0")
.type(ChangeRequestDecisionResourceType.CHANGE_REQUEST_DECISION)))))
.type(ChangeRequestResourceType.CHANGE_REQUEST))
.included(Collections.singletonList(new ChangeRequestDecisionCreateItem()
.attributes(new ChangeRequestDecisionCreateAttributes()
.changeRequestStatus(ChangeRequestDecisionStatusType.REQUESTED)
.requestReason("Please review and approve this change"))
.id("decision-id-0")
.relationships(new ChangeRequestDecisionCreateRelationships()
.requestedUser(new ChangeRequestUserRelationship()
.data(new ChangeRequestUserRelationshipData()
.id("00000000-0000-0000-0000-000000000000")
.type("user"))))
.type(ChangeRequestDecisionResourceType.CHANGE_REQUEST_DECISION)));

    try {
      ChangeRequestResponse result = apiInstance.updateChangeRequestDecision("CHM-1234", "decision-id-0", body);
