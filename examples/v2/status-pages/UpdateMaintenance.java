...
public class Example {
  public static void main(String[] args) {
    ...
    PatchMaintenanceRequest body = new PatchMaintenanceRequest()
.data(new PatchMaintenanceRequestData()
.attributes(new PatchMaintenanceRequestDataAttributes()
.scheduledDescription("We will be performing maintenance on the API to improve performance for 40 minutes.")
.inProgressDescription("We are currently performing maintenance on the API to improve performance for 40 minutes."))
.id(MAINTENANCE_DATA_ID)
.type(PatchMaintenanceRequestDataType.MAINTENANCES));

    try {
      Maintenance result = apiInstance.updateMaintenance(STATUS_PAGE_DATA_ID, MAINTENANCE_DATA_ID, body);
