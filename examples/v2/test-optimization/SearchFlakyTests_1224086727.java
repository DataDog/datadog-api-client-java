...
.type(FlakyTestsSearchRequestDataType.SEARCH_FLAKY_TESTS_REQUEST));
    ...
    try {
      PaginationIterable<FlakyTest> iterable = apiInstance.searchFlakyTestsWithPagination(new SearchFlakyTestsOptionalParameters().body(body));

      for (FlakyTest item : iterable) {
       System.out.println(item);
      }
    } catch (RuntimeException e) {
      System.err.println("Exception when calling TestOptimizationApi#searchFlakyTestsWithPagination");
