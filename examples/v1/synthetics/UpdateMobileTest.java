...
public class Example {
  public static void main(String[] args) {
    ...
    SyntheticsMobileTest body = new SyntheticsMobileTest()
.config(new SyntheticsMobileTestConfig()
.variables(Collections.singletonList(new SyntheticsConfigVariable()
.name("VARIABLE_NAME")
.secure(false)
...
.options(new SyntheticsMobileTestOptions()
.bindings(Collections.singletonList(new SyntheticsTestRestrictionPolicyBinding()
...
.deviceIds(Collections.singletonList("synthetics:mobile:device:apple_ipad_10th_gen_2022_ios_16"))
.mobileApplication(new SyntheticsMobileTestsMobileApplication()
.applicationId("00000000-0000-0000-0000-aaaaaaaaaaaa")
.referenceId("00000000-0000-0000-0000-aaaaaaaaaaab")
.referenceType(SyntheticsMobileTestsMobileApplicationReferenceType.LATEST))
...
.steps(Collections.singletonList(new SyntheticsMobileStep()
.name("")
.params(new SyntheticsMobileStepParams()
.check(SyntheticsCheckType.EQUALS)
.direction(SyntheticsMobileStepParamsDirection.UP)
.element(new SyntheticsMobileStepParamsElement()
.contextType(SyntheticsMobileStepParamsElementContextType.NATIVE)
.relativePosition(new SyntheticsMobileStepParamsElementRelativePosition())
.userLocator(new SyntheticsMobileStepParamsElementUserLocator()
.values(Collections.singletonList(new SyntheticsMobileStepParamsElementUserLocatorValuesItems()
.type(SyntheticsMobileStepParamsElementUserLocatorValuesItemsType.ACCESSIBILITY_ID)))))
.positions(Collections.singletonList(new SyntheticsMobileStepParamsPositionsItems()))
.variable(new SyntheticsMobileStepParamsVariable()
.example("")
.name("VAR_NAME")))
