...
.formulas(new ArrayList<ScatterplotWidgetFormula>() {{add(new ScatterplotWidgetFormula()
.formula("query1")
.dimension(ScatterplotDimension.X)
.alias("my-query1")); add(new ScatterplotWidgetFormula()
.formula("query2")
...
.aggregator(FormulaAndFunctionMetricAggregation.AVG))); add(new FormulaAndFunctionQueryDefinition(
new FormulaAndFunctionMetricQueryDefinition()
.dataSource(FormulaAndFunctionMetricDataSource.METRICS)
.name("query2")
