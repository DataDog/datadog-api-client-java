...
.type(SLOType.METRIC)
.description("string")
.groups(new ArrayList<String>() {{add("env:test"); add("role:mysql");}})
.monitorIds(new ArrayList<Long>() {{;}})
.name("Example-Create_an_SLO_object_returns_OK_response")
.query(new ServiceLevelObjectiveQuery()
.denominator("sum:httpservice.hits{!code:3xx}.as_count()")
.numerator("sum:httpservice.hits{code:2xx}.as_count()"))
.tags(new ArrayList<String>() {{add("env:prod"); add("app:core");}})
.thresholds(new ArrayList<SLOThreshold>() {{add(new SLOThreshold()
.target(95.0)
.targetDisplay("95.0")
