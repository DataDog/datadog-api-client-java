...
def pytest_bdd_after_scenario(request, feature, scenario):
    ...
    output = ROOT_PATH / "examples" / version / group_name / f"{operation_id}{unique_suffix}.java"
...
TIME_FORMATTER = {
    ...
    "timestamp": "{sret}.toInstant().getEpochSecond()",
    "isoformat": "{sret}",
    "units": {
        "s": "{sret}.plusSeconds({num})",
        "m": "{sret}.plusMinutes({num})",
        "h": "{sret}.plusHours({num})",
        "d": "{sret}.plusDays({num})",
        "M": "{sret}.plusMonths({num})",
        "y": "{sret}.plusYears({num})",
    },
}
...
def relative_time(imports, calls, freezed_time, iso):
    ...
    def func(arg):
        imports["java.time"].add("OffsetDateTime")
